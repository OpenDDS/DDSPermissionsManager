							...
							<table data-cy="topics-table" class="main" style="margin-top: 0.5rem">
								<thead>
									<tr style="border-top: 1px solid black; border-bottom: 2px solid">
										...
										{/if}
										<td class="header-column" style="min-width: 7rem">{messages['topic-sets']['table.column.one']}</td>
										<td class="header-column">{messages['topic-sets']['table.column.two']}</td>
									</tr>
								</thead>
								<tbody>
									{#each $topicSets as topicSet (topicSet.id)}
										<tr>
											{#if $permissionsByGroup?.find((gm) => gm.groupName === $groupContext?.name && gm.isTopicAdmin === true) || $isAdmin}
